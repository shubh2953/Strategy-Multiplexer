...
from ibind import IbkrClient, ibind_logs_initialize, QuestionType
...
class IBKRClientWrapper:
    def __init__(self):
        # Use OAuth authentication with environment variables
        self.client = IbkrClient(use_oauth=True)
    ...
    def place_order(self, action, quantity, symbol, order_type="MOC",tif="DAY"):
        if not self.connected or not self.account_id:
            print("Not connected to IBKR or no account ID available")
            return None
        ...
        try:
            ...
            # search_results = self.client.search_contract_by_symbol(symbol).data
            ...
            conid = int(search_results[0]['conid'])  # Convert to integer
            ...
            answers = {
                ...
                'size exceeds the Size Limit of': True,
                'Unforeseen new question': True,
            }

            # Place the order
            response = self.client.place_order(order_request, answers, self.account_id).data
            print(f"Order placed: {response}")

            # Store order ID for future reference
            self.order_status[order_tag] = {
                'status': 'Submitted',
                'filled': 0,
                'remaining': quantity,
                'avgFillPrice': 0
    ...
    def request_account_summary(self):
        try:
            ...
            for currency, subledger in ledger.items():
                account_info = {
                    'NetLiquidation': {
                        'value': subledger.get('netliquidationvalue', 0),
                        'currency': currency
                    },
                    'TotalCashValue': {
                        'value': subledger.get('cashbalance', 0),
                        'currency': currency
                    },
                    'AvailableFunds': {
                        'value': subledger.get('availablefunds', 0),
                        'currency': currency
                    }
                }
                self.account_summary[self.account_id] = account_info

            print("Account summary updated")
            return True
...
def connect(self):
    try:
        ...
        if accounts:
            ...
            self.client.account_id = self.account_id
            print(f"Connected to IBKR Web API. Using account: {self.account_id}")
            self.connected = True  # This flag is used for connection status
            return True
        else:
            print("No accounts found")
            return False
...
def main():
    ...
    wait_time = 0
    max_wait = 30
    while not app.connected and wait_time < max_wait:
        time.sleep(1)
        wait_time += 1
    ...
    try:
        ...
        timer = threading.Timer(60 * 60, delayed_full_update)
        timer.start()
